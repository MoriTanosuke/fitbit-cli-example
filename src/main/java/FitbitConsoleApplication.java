...
public class FitbitConsoleApplication {
	...
	public static void main(String[] args) throws FitbitAPIException,
		...
		TimeSeriesResourceType[] typesToLoad = new TimeSeriesResourceType[] {
				...
				TimeSeriesResourceType.ELEVATION, TimeSeriesResourceType.FAT,
				...
				TimeSeriesResourceType.WATER, TimeSeriesResourceType.WEIGHT };
		...
		for (TimeSeriesResourceType type : typesToLoad) {
			data.put(
					type,
					loadData(localUser, client, startDate, fitbitUser, type,
							period));
		}
	...
	private static void save(LocalUserDetail localUser,
		...
		try {
			...
			out.write(fitbit.getResourceCredentialsByUser(localUser)
					.getAccessToken());
			out.close();
			FileWriter out2 = new FileWriter("secret.properties");
			out2.write(fitbit.getResourceCredentialsByUser(localUser)
	...
	private static List<Data> loadData(final LocalUserDetail localUser,
		...
		List<Data> timeSeries = client.getTimeSeries(localUser, fitbitUser,
				type, startDate, period);
		/*
	...
	private static String today(Date date) {
		return new SimpleDateFormat(FitbitApiService.LOCAL_DATE_PATTERN)
				.format(date);
	}
